...
document.addEventListener('DOMContentLoaded', function() {
    ...
    formUser.onsubmit = function(e) {
        ...
        if (strNome == '' || strUsuario == '' || strRol == '' || intStatus == '') {
            swal("Atencion", "Todos os campos são necesarios", "error");
            return false;
        }
    ...
    btnDelUser.forEach(function(btnDelUser) {
        btnDelUser.addEventListener('click', function() {
            ...
            swal({
                ...
                text: "Realmente deseja excluir o usuario?",
                type: "warning",
                ...
                if (Confirm) {
                    ...
                    request.onreadystatechange = function() {
                        if (request.readyState == 4 && request.status == 200) {
                            var objData = JSON.parse(request.responseText);
                            if (objData.status) {
                                swal("Pronto!", objData.msg, "success");
                                tableUsuarios.ajax.reload(function() {
                                    editUser();
                                    delUser();
                                });
